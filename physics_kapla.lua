...
function add_kapla_tower(scn, resources, width, height, length, radius, material, level_count, x, y, z)
	...
	for i=0, level_count // 2 -1 do
		function fill_ring(r, ring_y, size, r_adjust, y_off)
			step = math.asin((size * 1.01) / 2 / (r - r_adjust)) * 2
			cube_count = (2 * math.pi) // step
			error = 2 * math.pi - step * cube_count
			step = step + error / cube_count  -- distribute error

			a = 0
			while a < (2 * math.pi - error) do
				world = hg.TransformationMat4(hg.Vec3(math.cos(a) * r + x, ring_y, math.sin(a) * r + z), hg.Vec3(0, -a + y_off, 0))
