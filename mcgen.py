...
def buildmodel(vtx_layout, chunk_size, chunk_pos):
	list_mats = []
	final_list_mats = [[], [], []]
...
while not hg.ReadKeyboard().Key(hg.K_Escape):
	...
	if chunk_index < len(queue):
		mdl = buildmodel(vtx_layout, chunk_size, hg.Vec3(queue[chunk_index][0] * chunk_size, queue[chunk_index][1] * chunk_size, queue[chunk_index][2] * chunk_size))
